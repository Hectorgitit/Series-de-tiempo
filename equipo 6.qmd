...
**Outlier** si se cumple:

Sea $r_t$ el **residuo** en el tiempo t.

Definimos:

-   $Q_1=quantile(r_t,0.25)$

-   $Q_3=quantile(r_t,0.75)$

-   $IQR=Q_3-Q_1$

    Un residuo será considerado **outlier** si está demasiado lejos del rango central $[Q_1,Q_3]$
...
ventas_tienda_future <- new_data(ventas_tienda_train, n = h_ventas + 10) |>
  mutate(
...
\[\En este proyecto desarrollamos un flujo completo de pronóstico para las ventas horarias de la Tienda Delicatessen, comenzando con el tratamiento de outliers mediante una descomposición STL robusta e interpolación, y continuando con la construcción de una tsibble horaria enriquecida con variables de calendario. A partir de esta serie limpia, ajustamos y comparamos diversos modelos de referencia (MEAN, NAIVE, SNAIVE), modelos ETS y modelos basados en descomposición estacional y regresión.

Tras evaluar el desempeño de todos los modelos mediante el MAE en el conjunto de prueba, seleccionamos como modelo final stl_arima_robusto, ya que fue el que obtuvo el MAE más bajo entre las alternativas consideradas. Con este modelo generamos el pronóstico horario para noviembre de 2025 y construimos la tabla final de pronósticos, cumpliendo con los entregables y criterios establecidos para el proyecto.\]
