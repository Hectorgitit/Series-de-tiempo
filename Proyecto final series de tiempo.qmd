...
ventas_tienda_fit <- ventas_tienda_train |>
  model(
    # 1) Seasonal naive (por ejemplo, estacionalidad diaria)
    snaive_day = SNAIVE(total ~ lag("day")),
    ...
    # 5) Regresión armónica dinámica (Fourier + ARIMA)
    #dhr = ARIMA(
      #total ~ fourier("day", K = 5) +
               #fourier("week", K = 3) +
               #work_day
    #),
    ...
    # 7) Combinación simple (promedio de varios modelos)
    combo = (ETS(total) + ARIMA(total) +
             prophet(total ~ growth("linear") + season("week") + season("year"))
            ) / 3
  )

report(ventas_tienda_fit)
