...
## PD
Intente bajar series mas razonables como la capacidad eolica y la capacidad de intalada de paneles, pero hubo un tema con la pagina de U.S. Energy Information Administration y ni con mi API key pude bajar nada 😭
...
U.S. Energy Information Administration (EIA), que es la agencia oficial de estadísticas de energía de Estados Unidos, menciona que para la generacion electrica en EE.UU. se usa.
...
var_out <- "CPIENGSL"   # <-- cambia aquí el nombre a eliminar
resp <- all.vars(fmla_arimax)[1]
xs   <- setdiff(all.vars(fmla_arimax)[-1], var_out)
stopifnot(length(xs) > 0)
fmla_arimax <- reformulate(xs, response = resp)
...
# 1) Recojo Y y las X vigentes (por si quitaste algunas en “Optimización”)
...
# 3) Serie histórica de X (en largo), recortada al mismo horizonte del ARIMAX
x_series <- energia_mensual_tsb |>
  dplyr::filter(
    serie %in% x_vars,
    mes <= ultimo_hist          # 👈 recortamos aquí
  )
...
# 5) Pronóstico ARIMA de cada X usando h = h_meses
x_fc <- x_fit |>
...
# Guardamos una versión tibble alineada para usarla en los escenarios
x_fc_aligned <- new_data_exante |>
  dplyr::as_tibble()

# 7) Ex–ante (escenario base) de Y con ARIMAX
...
```{r graf_base}
#| warning: false
# Serie histórica + pronóstico base con bandas de confianza
...
fc_exante_plt <- fc_exante |>
  ...
  labs(
    title = "Pronóstico ex–ante (escenario base)",
    x = NULL,
    y = y_var
  )
...
fc_scen_medias_plt <- fc_scen_medias |>
  ...
  ggplot2::geom_line() +
  labs(
    title = "Media de los pronósticos por escenario",
    x = NULL,
    y = y_var,
    colour = "Escenario"
  )
...
```{r arimax_insample}
# Pronóstico in-sample del ARIMAX (ajuste sobre toda la muestra)

arimax_insample <- energia_arimax_fit |>
  fabletools::augment()

# Imprimimos solo fecha, valor observado y valor ajustado
arimax_insample |>
  dplyr::select(mes, dplyr::all_of(y_var), .fitted)

```

```{r graf_insample}
# Pronóstico in-sample: observada vs ajustada por el ARIMAX

arimax_insample <- energia_arimax_fit |>
  fabletools::augment()

graf_insample <- arimax_insample |>
  ggplot2::ggplot(aes(x = mes)) +
  ggplot2::geom_line(aes(y = .data[[y_var]])) +
  ggplot2::geom_line(aes(y = .fitted), linetype = "dashed") +
  ggplot2::labs(
    title = "ARIMAX – ajuste in-sample",
    x = NULL,
    y = y_var
  )
